...
class Eformance_KomerciParc_PayController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        return Mage::getSingleton('Eformance_KomerciParc/payment');
    }
  ...
  public function getOrder()
    ...
    public function redirectAction()
    {
        ...
        $komerce = $this->getKomerci(); //pega todos os metodos do modelo payment.php
	...
	$payment = $order->getPayment(); //pega os dados do pagamento
        ...
        $webservice = Mage::getModel('Eformance_KomerciParc/Komerci');
        /*===========================================================================================
  ...
  $taxa_juros = Mage::getStoreConfig('payment/Eformance_KomerciParc/parcelamento_juros');
		...
		$juros                  = $komerce->getConfigData('parcelamento_juros', $storeId);
        $parcelasMaximo         = $komerce->getConfigData('num_max_parc', $storeId);
        $parcelasSemJuros       = $komerce->getConfigData('parcelamento_semjuros', $storeId);
        $parcelasValorMinimo    = $komerce->getConfigData('valor_minimo', $storeId);
        $descontoAVista         = $komerce->getConfigData('desconto_avista', $storeId);
        $descontoAVistaValor    = $komerce->getConfigData('valor_desconto_avista', $storeId);
        ...
        if($parcelas == 0){
            ...
            $transacao = $komerce->getConfigData('tipo_parcelamento', $storeId);$order->getId(); //verifica se é emissor ou loja
        }
        $filiacao  = $komerce->getConfigData('filiacao', $storeId);
        $numpedido = $orderId;
     ...
     /*echo $total ."<br>";
        ...
        if($autorizacao['codret'] == 0)  
        {
            ...
            $this->_redirect('checkout/onepage/failure/'.$autorizacao['codret'].'/motivo/'.$autorizacao['msgret'].'/orderid/'.$orderId);
        }
